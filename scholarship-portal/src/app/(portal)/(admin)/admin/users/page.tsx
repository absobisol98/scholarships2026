        ...
        <div className="card elev-sm" style={{ marginTop: "var(--space-6)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)", marginTop: "var(--space-2)" }}>
            {admins.map((a) => {
              ...
              return (
                <div key={a.id} style={{ padding: "10px 0", borderBottom: "1px solid var(--color-divider)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "var(--space-3)", flexWrap: "wrap" }}>
                    ...
                    <form action={onToggle}>
                      <button type="submit" className="btn btn-secondary" style={{ padding: "6px 12px" }}>{a.active ? "Deactivate" : "Reactivate"}</button>
                    </form>
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center", marginTop: 8 }}>
                    <span className="text-muted" style={{ fontSize: 11 }}>Programs:</span>
        ...
        <div className="card elev-sm" style={{ marginTop: "var(--space-4)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)", marginTop: "var(--space-2)" }}>
            {screeners.map((s) => {
              ...
              return (
                <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "var(--space-3)", padding: "8px 0", borderBottom: "1px solid var(--color-divider)", flexWrap: "wrap" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span className="text-muted" style={{ fontSize: 12 }}>{s.applicantAssignments.length} applicant{s.applicantAssignments.length === 1 ? "" : "s"} assigned</span>
                  </div>
                  <form action={onToggle}>
                    <button type="submit" className="btn btn-secondary" style={{ padding: "6px 12px" }}>{s.active ? "Deactivate" : "Reactivate"}</button>
                  </form>
                </div>
              );
            })}
